...
import {projects , services} from "../assets/data"
      ...
      <div className=" relative  ">
        <div className=" relative lg:h-[95vh] bg-[url('/our_specialties_header_background_image.png')]">
          ...
          <div className=" relative overflow-hidden" >
            <div className="flex overflow-hidden flex-col pb-4 lg:pb-0 lg:flex-row relative translate-x-7 lg:translate-x-44 lg:w-[1000px]">
                {
                    services.map(e => {
                        return (
                            <div key={e} className={`overflow-hidden relative duration-[2s] ${item === e.title ? "lg:w-[800px] w-[600px] h-[250px] lg:h-[350px]":"bg-secondary w-[600px] lg:w-[400px] h-[60px] lg:h-[350px]"} w-[400px] border `} onMouseEnter={() => setItem(e.title)}>
                            <h3 className={`${item === e.title ? " text-primary  text-center translate-y-32 lg:translate-y-40 translate-x-56 lg:translate-x-9 lg:rotate-0 font-bold text-[16px]":" text-[16px] font-semibold text-white lg:rotate-[-90deg] translate-x-[250px] lg:translate-x-[140px] w-[160px]"} leading-8 z-10 relative   w-[160px] h-full duration-[2s] `}>{e.title}</h3>
                            {
        ...
        <div className=" flex mx-10 lg:mx-40  my-20 lg:gap-[65px] gap-6 flex-wrap">
                        ...
                        return (
                            <div key={e} onClick={()=> {setProject(e) , setVisibleBottom(true)}} className="">
                                ...
                                <div className="bg-[#1B2125] px-4 py-2  rounded-b-[8px]">
                                    ...
                                    <p className=" text-[#F4F4F4]/50 font-light text-[14px]">{e.decs}</p>
                                </div>
                            </div>
                        )
                    })
                }
                <Sidebar visible={visibleBottom} className="h-screen bg-secondary" position="bottom" onHide={() => setVisibleBottom(false)}>
                <div>
                    <div>
                        ...
                        <div className=" flex flex-col-reverse lg:flex-row mt-8 mx-9">
                            <div className=" flex max-h-fit h-fit">
                               ...
                               <h3 className=" text-white font-semibold text-xl ">the Challenge:</h3>
                                <p className="text-white/50 font-light mr-36">{project?.Challenge}</p>
                                <h3 className=" text-white font-semibold text-xl mt-4 ">The Solution:</h3>
                                <p className="text-white/50 font-light mr-36">{project?.Solution}</p>
                                <p className="text-white/50 font-light mr-36 mt-4">{project?.text}</p>
                               </div>
                            </div>
                           <div className=" min-w-[400px] lg:mb-0 mb-4">
                           <img className=" border border-[#ddd]/50 w-full mx-auto lg:mx-0 rounded-[8px]" src={project?.image} alt="" />
                           <button className=" bg-primary py-3 rounded-[8px] text-white mt-4 w-full">Watch the full Case Study</button>
                           </div>
                        </div>
                    </div>
                </div>
